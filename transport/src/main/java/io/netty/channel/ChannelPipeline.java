...
public interface ChannelPipeline
    ...
    /**
     * Netty的Channel过滤器实现原理与Servlet Filter机制一致,它将Channel的数据管道抽象为ChannelPipeline,
     * 消息在ChannelPipeline中流动和传递;ChannelPipeline持有I/O事件拦截器ChannelHandler的链表,
     * 由ChannelHandler对I/O事件进行拦截和处理,可以方便的通过新增和删除ChannelHandler来实现不同的业务逻辑定制,
     * 不需要对已有的ChannelHandler进行修改,能够实现对修改封闭对扩展开放的支持;
     */

    /**
     * 消息的读取和发送处理全流程:
     * 1. 底层的SocketChannel read()读取ByteBuf,触发ChannelRead事件,
     * 由I/O线程NioEventLoop调用ChannelPipeline的fireChannelRead(Object msg)方法,将消息传输到ChannelPipeline中;
     * 2. 消息依次被HeadHandler,ChannelHandler1,ChannelHandler2,...TailHandler拦截处理,
     * 在这个过程中,任何ChannelHandler都可以中断当前的流程,结束消息的传递;
     * 3. 调用ChannelHandlerContext的write()发送消息,消息从TailHandler开始,途径ChannelHandlerN...ChannelHandler1,HeadHandler,
     * 最终被添加到消息发送缓冲区中等待刷新和发送,在此过程中也可以中断消息的传递,例如当编码失败时,就需要中断流程,构造异常的Future返回;
     */
     ...
     * 4. ChannelHandlerContext.fireChannelReadComplate(): 读操作完成通知事件;
     * 5. ChannelHandlerContext.fireExceprionCaught(Throwable): 异常通知事件;
     ...
     * 8. ChannelHandlerContext.fireChannelInactive(): tcp链路关闭,链路不可用通知事件;
     *
