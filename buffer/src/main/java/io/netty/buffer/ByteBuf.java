...
/**
 * TODO 抽象类,Netty的数据容器,扩展jdk的ByteBuffer
 * 是Netty中最重要,最基础的数据结构;
 * 主要操作类似增删改查
 * 0x00:NULL
 */
...
public abstract class ByteBuf implements ReferenceCounted, Comparable<ByteBuf> {
	 ...
	 * 4. 在读和写两种模式间切换不需要调用ByteBuffer的flip();
	 * 5. 读和写使用了不通过的索引;
	 * 6. 支持方法的链式调用;
	 * 7. 支持引用计数;
	 * 8. 支持池化;
	 *
	 * ByteBuf使用:
	 * 1. 堆缓冲区:支撑数组,数据存储在jvm的堆空间中,能在没有使用池化的情况下提供快速的分配和释放;
	 * 2. 直接缓冲区: jdk的ByteBuffer类运行jvm实现通过本地调用来分配内存,这主要是为了避免在每次调用本地I/O操作之前(或之后)
	 * 将缓冲区的内容复制到一个中间缓冲区(或者从中间缓冲区复制到缓冲区);
	 * ByteBuffer的javadoc指出:"直接缓冲区的内容将驻留在常规的会被垃圾回收的堆之外",
	 * 这是为何直接缓冲区是对于网络数据传输的最理想选择;直接缓冲区的缺点是:相对于基于堆的缓冲区,它们的分配和释放都较为昂贵;
	 * 3. 复合缓冲区: 为多个ByteBuf提供了一个聚合视图,我们可能需要添加或者删除ByteBuf实例,jdk的ByteBuffer没有这个特性,
	 * Netty通过一个ByteBuf的子类--CompositeByteBuf实现了这一特性,提供了一个将多个缓冲区表示为单个合并缓冲区的虚拟表示;
	 * 		CompositeByteBuf中的ByteBuf实例可能同时包含直接内存分配和非直接内存分配;
	 *
	 * 	ByteBuf字节级操作,提供了许多超出基本读,写操作的方法用于修改它的数据;
	 * 	1. 随机访问索引;
	 * 	2. 顺序访问索引;
	 * 	3. 可丢弃字节;
	 * 	4. 可读字节;
